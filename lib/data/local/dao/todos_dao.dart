...
class TodosDao extends DatabaseAccessor<AppDatabase> with _$TodosDaoMixin {
  ...
  Future<List<TagRow>> getTagsForTodo(String todoId) async {
    final junctions = await (select(db.todoTagsTable)
          ..where((j) => j.todoId.equals(todoId)))
        .get();
      ...
      if (tagIds.isNotEmpty) {
        await batch((b) {
          b.insertAllOnConflictUpdate(
            db.todoTagsTable,
            tagIds
                .map((tid) => TodoTagsTableCompanion.insert(
                      todoId: todoId,
                      tagId: tid,
                    ))
                .toList(),
  ...
  Future<void> cleanJunctionsForDeletedTodos(List<String> tombstoneIds) async {
    ...
    await (delete(db.todoTagsTable)
          ..where((j) => j.todoId.isIn(tombstoneIds)))
        .go();
  }
  ...
  Future<bool> instanceExistsForDate(
      String templateId, String dateOnly) async {
    final row = await (select(db.todosTable)
          ..where((t) =>
              t.recurrenceTemplateId.equals(templateId) &
              t.scheduledDate.equals(dateOnly) &
              t.deletedAt.isNull())
  ...
  Future<void> softDeleteFutureInstances(
      String templateId, String fromDateInclusive, String deletedAtIso) {
    return (update(db.todosTable)
          ..where((t) =>
              t.recurrenceTemplateId.equals(templateId) &
  ...
  Future<void> softDeleteAllInstances(
      String templateId, String deletedAtIso) {
    return (update(db.todosTable)
          ..where((t) =>
              t.recurrenceTemplateId.equals(templateId) &
              t.deletedAt.isNull()))
