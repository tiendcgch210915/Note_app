...
///  - Computed/server-only fields are not sent.
...
class SyncPayload {
  ...
  static Map<String, dynamic> fromTodo(
    ...
    return {
      ...
      'recurrence_interval': isSubtask ? null : row.recurrenceInterval,
      'recurrence_days_of_week': isSubtask ? null : row.recurrenceWeekdays,
  ...
  static Map<String, dynamic> fromChecklistCategory(ChecklistCategoryRow row) =>
      {
        'id': row.id,
        'user_id': row.userId,
        'name': row.name,
  ...
  static Map<String, dynamic> fromTemplateItem(TemplateItemRow row) => {
    ...
    'position':
        row.orderIndex, // Drift column = orderIndex, contract key = position
    'created_at': row.createdAt,
    'updated_at': row.updatedAt,
    'deleted_at': row.deletedAt,
  };
  ...
  static Map<String, dynamic> pushOperation({
    required String op,
    required String type,
    required Map<String, dynamic> payload,
  }) => {'op': op, 'type': type, 'payload': payload};
...
List<T> sortedByDependency<T>({
  ...
  return [...rows]..sort((a, b) {
    final ai = _entityOrder
        .indexOf(getEntityType(a))
        .let((i) => i < 0 ? 99 : i);
    final bi = _entityOrder
        .indexOf(getEntityType(b))
        .let((i) => i < 0 ? 99 : i);
